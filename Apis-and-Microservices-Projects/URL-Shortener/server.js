			...
			mongodb.connect(url, function( err, db ){
				...
				collection.find({
					...
					if(results.length === 0) {
						...
						res.send({ 'original_url': urlParam, 'shortLink' : 'https://daa-url-shortener-microservice.glitch.me/' +randomNbr });

					} else {
	    				
	    				// console.log(results); // output all records
	    				res.send({ 'original_url': results[0].url, 'shortLink' : 'https://daa-url-shortener-microservice.glitch.me/' + results[0].randomNbr });
					}
